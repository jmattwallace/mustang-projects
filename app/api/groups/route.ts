...
export async function PATCH(request: NextRequest) {
  ...
  const { error } = await db.from("project_groups").update({ name: String(body.name).trim(), color: body.color || "#2763d9" }).eq("id", body.id).eq("creator_id", user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 }); return NextResponse.json({ ok: true }, { headers: response.headers });
}
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await db.from("project_groups").delete().eq("id", id).eq("creator_id", user.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 }); return NextResponse.json({ ok: true }, { headers: response.headers });
}
