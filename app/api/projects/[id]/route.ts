...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params; const body = (await request.json()) as { title?: string; completion?: number; gross?: number; net?: number; actualPaid?: number; paidInFull?: boolean; targetDate?: string | null; stages?: unknown };
