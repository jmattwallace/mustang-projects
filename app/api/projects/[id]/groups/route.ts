...
export async function PUT(request: NextRequest, { params }: { params: Promise<{id:string}> }) {
  ...
  const { id } = await params; const { groupIds } = await request.json();
  const { error: ownership } = await db.from("projects").select("id").eq("id", id).eq("owner_id", user.id).single(); if (ownership) return NextResponse.json({ error: "Only the owner can change groups." }, { status: 403 });
