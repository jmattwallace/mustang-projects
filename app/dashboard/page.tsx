...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ viewAs?: string }> }) {
  ...
  const { data: groups } = await supabase.from("project_groups").select("id,name,color").eq("creator_id", boardOwnerId).order("name");
  const { data: arrangements } = await supabase.from("saved_arrangements").select("id,name,positions").eq("owner_id", boardOwnerId).order("created_at");
